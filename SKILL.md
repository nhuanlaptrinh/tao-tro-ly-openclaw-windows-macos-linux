...
description: Skill tự chứa để cài đặt, tạo, cấu hình hoặc vận hành trợ lý OpenClaw trên Windows, macOS và Linux, gồm máy cá nhân, VPS chính và VPS thành viên chạy Docker. Bao gồm workflow Node.js/Python/OpenClaw local, Gateway, Codex login, DeepSeek tùy chọn, Token Codex, Telegram, Zalo QR/voice/reliability, kiểm tra openclaw.json, Custom Provider, proxy fallback, Second AI Brain, audio, web search, xử lý ảnh và tích hợp Facebook Fanpage/CSKH; không yêu cầu cài hay đọc skill phụ.
---
...
4. Trên Linux VPS chính, xác định runtime, service/container, `HOME` và workspace OpenClaw đang hoạt động bằng process/service/config thực tế; không hardcode một đường dẫn runtime nếu chưa tồn tại.
...
- Workspace bên trong container: `/workspace`
- Home OpenClaw bên trong container: `/home/<name>`; đặt `HOME=/home/<name>` cho mọi lệnh OpenClaw
...
2. Không cài OpenClaw, Telegram, Zalo, Nginx bổ sung, document tools, proxy hoặc Second AI Brain khi user chỉ yêu cầu container nền.
...
- Chạy gateway bằng home thật của member và luôn nạp `/home/<name>/.openclaw/token-codex.env` trước khi chạy; nếu không, `${TOKEN_CODEX_API_KEY}` sẽ rỗng khi Gateway reload hoặc khởi động lại.
...
- Khi tạo/cấu hình trợ lý mới, trong `/home/<name>/.openclaw/openclaw.json` của container member VPS phải có `plugins.entries.duckduckgo.enabled = true` và `tools.web.search.provider = "duckduckgo"`.
...
3. Lưu file trong `/home/<ten_user>/.openclaw/workspace` của container member VPS.
...
Nếu người dùng chỉ cần container nền, dùng đúng interface hiện tại:
...
Lệnh trên chỉ map SSH. Với member OpenClaw đầy đủ, trước khi cài phải xác minh container có thêm mapping `<web_port>:80`, volume `/root/docker-users/data/<ten_user>/home:/home/<ten_user>` và workspace `/root/docker-users/data/<ten_user>/workspace:/workspace`. Nếu provisioning hiện tại không tạo được các mapping này, không được báo dashboard public hoặc OpenClaw member hoàn tất; phải sửa provisioning riêng sau khi backup.
...
Không ghi token thật vào skill, README, nhật ký, câu trả lời, Git hoặc file dùng chung. Nếu lưu Telegram settings, token/chat ID chỉ nằm trong home thật `/home/<ten_user>/.openclaw/` với quyền `600`.
...
```bash
docker exec -i -e HOME=/home/<ten_user> user-<ten_user> node <<'NODE'
const fs = require('fs');
const path = '/home/<ten_user>/.openclaw/openclaw.json';
...
```bash
docker exec user-<ten_user> sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw config validate'
docker exec user-<ten_user> tmux kill-session -t openclaw 2>/dev/null || true
docker exec user-<ten_user> tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<ten_user> openclaw gateway run'
sleep 8
docker exec user-<ten_user> sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw gateway status'
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc 'journalctl --user -u openclaw-gateway.service --since "10 minutes ago" --no-pager -o cat 2>/dev/null | rg -i "telegram:group|inbound|outbound send ok" || true'
```
...
```bash
docker exec -e HOME=/home/<ten_user> user-<ten_user> openclaw pairing approve --channel telegram --account <ten_user> <PAIRING_CODE>
```
...
docker exec -e HOME=/home/<ten_user> user-<ten_user> openclaw --version
docker exec -e HOME=/home/<ten_user> user-<ten_user> openclaw channels status --probe
```
...
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc \
  'tar -C /home/<ten_user>/.openclaw -czf - openclaw.json $(test -d credentials/zalouser && printf credentials/zalouser || true)' \
...
```bash
docker exec -e HOME=/home/<ten_user> user-<ten_user> \
  openclaw plugins install --pin '@openclaw/zalouser@2026.7.1'
docker exec -e HOME=/home/<ten_user> user-<ten_user> openclaw config validate
```
...
- Đọc `/home/<ten_user>/.openclaw/openclaw.json` để tìm Telegram account đang enabled.
...
```bash
docker exec -it -e HOME=/home/<ten_user> user-<ten_user> \
...
```text
/home/<ten_user>/.openclaw/credentials/zalouser/credentials.json
```
...
```bash
docker exec -e HOME=/home/<ten_user> user-<ten_user> openclaw pairing list zalouser
docker exec -e HOME=/home/<ten_user> user-<ten_user> \
...
docker exec user-<ten_user> tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<ten_user> openclaw gateway run'
sleep 8
docker exec user-<ten_user> sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw channels status --probe'
```
...
```bash
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc 'apt update && apt install tmux -y'
docker exec user-<ten_user> tmux kill-session -t openclaw 2>/dev/null || true
docker exec user-<ten_user> tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<ten_user> openclaw gateway run'
```
...
```bash
docker exec user-<ten_user> sh -lc 'tmux ls; set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw gateway status; HOME=/home/<ten_user> openclaw channels status --probe'
```

## Cấu hình Custom Provider Token Codex

Tham khảo schema từ một member đang chạy sau khi kiểm tra bằng `docker inspect`; không giả định member mẫu hoặc đường dẫn host cố định. Khi cấu hình member VPS mới, sửa config bên trong container `user-<ten_user>` tại `/home/<ten_user>/.openclaw/openclaw.json` với `HOME=/home/<ten_user>`.

Không ghi API key thật vào skill, README, nhật ký hoặc câu trả lời. Với member mới, lưu key vào `/home/<ten_user>/.openclaw/token-codex.env` quyền `600`, rồi chỉ ghi chuỗi `${TOKEN_CODEX_API_KEY}` vào `openclaw.json`:

```bash
docker exec -e TOKEN_CODEX_API_KEY user-<ten_user> sh -lc '
  test -n "$TOKEN_CODEX_API_KEY"
  umask 077
  printf "TOKEN_CODEX_API_KEY=%s\n" "$TOKEN_CODEX_API_KEY" > /home/<ten_user>/.openclaw/token-codex.env
  chmod 600 /home/<ten_user>/.openclaw/token-codex.env
'
```
...
docker exec user-<ten_user> tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<ten_user> openclaw gateway run'
sleep 8
docker exec user-<ten_user> sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw gateway status; HOME=/home/<ten_user> openclaw models list | head'
```
...
2. Kiểm tra file `/home/<ten_user>/.openclaw/token-codex.env` tồn tại, quyền `600` và biến `TOKEN_CODEX_API_KEY` không rỗng; chỉ in `token_env=present` hoặc `token_env=missing`.
...
4. Nếu `/models` trả `200` nhưng Gateway vẫn dùng provider/model cũ, chuyển cache `/home/<ten_user>/.openclaw/agents/main/agent/models.json` sang file backup có timestamp; không xóa không backup.
...
docker exec user-<ten_user> sh -lc '
  set -a
  . /home/<ten_user>/.openclaw/token-codex.env
  set +a
  HOME=/home/<ten_user> node <<'"'"'NODE'"'"'
...
Lệnh dưới đây tự kiểm tra `/models`, không in API key, xóa provider `openai` cũ nếu có và cập nhật `/home/<ten_user>/.openclaw/openclaw.json` an toàn:
...
docker exec user-<ten_user> tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<ten_user> openclaw gateway run'
sleep 8
docker exec user-<ten_user> sh -lc 'set -a; . /home/<ten_user>/.openclaw/token-codex.env; set +a; HOME=/home/<ten_user> openclaw gateway status; HOME=/home/<ten_user> openclaw channels status --probe'
```
...
Cấu hình cần có trong container `user-<ten_user>` tại `/home/<ten_user>/.openclaw/openclaw.json`:
...
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc 'openclaw --version && HOME=/home/<ten_user> openclaw gateway status && HOME=/home/<ten_user> openclaw channels status --probe'
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc 'python3 --version && document-python --version && command -v pdfinfo && command -v pdftotext'
docker exec -e HOME=/home/<ten_user> user-<ten_user> sh -lc '/home/<ten_user>/.openclaw/tools/document-venv/bin/python -c "import openpyxl,pypdf,pdfplumber,fitz,PIL,xlsxwriter,pandas; print(\"document_modules=OK\")"'
```
...
5. Sau khi sửa workspace hoặc config, restart Gateway bằng HOME thật của member:

```bash
HOME=/home/<username> tmux kill-session -t openclaw 2>/dev/null || true
tmux new-session -d -s openclaw sh -lc 'set -a; . /home/<username>/.openclaw/token-codex.env; set +a; exec env HOME=/home/<username> openclaw gateway run'
```
